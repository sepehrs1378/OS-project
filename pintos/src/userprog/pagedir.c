...
/* Reads a byte at user virtual address UADDR.
   ...
   Return the byte value is successful, -1 if a segfault occurred. */
...
/* Checks if the entity referenced to by PTR which has size LEN, is in
   the user space and it's valid (i.e. it is mapped to a page).
   If it's not, kills the user process. See "Accessing User Memory" in
   project's reference for more information. */
void validate(const void* ptr, int len) {
...
/* Is the String valid?
This means that if the string length exceeds its maximum size or is
null, the process will be killed.  */
void validate_string(const char* ptr, int max_len) {
  int i;
  for (i = 1; i < max_len; i++) {
    validate(ptr, i);
    if (ptr[i - 1] == '\0')
